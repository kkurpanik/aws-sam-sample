...
```bash
sam-py$ sam build --use-container
```
...
```bash
sam-py$ sam logs -n StockCheckerFunction --stack-name sam-py --tail
```
...
```bash
sam-py$ pip install -r tests/requirements.txt --user
# unit test
sam-py$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sam-py$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
